...
extern uint8_t flash_write(uint8_t* buff, uint32_t len);
extern void erase_sector_test(uint8_t sector_number);

extern uint32_t GetSector(uint32_t Address);
extern uint8_t BSP_FLASH_Read(uint8_t* buff, uint32_t len);
